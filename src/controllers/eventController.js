...
export const getTopWines = async (req, res) => {
    try {
        // Busca todos os rankings marcados como final
        ...
        const wines = rankingsSnapshot.docs.map((doc) => {
            const data = doc.data();
            return {
                wineId: data.wineId,
                name: data.name,
        ...
        const topWines = wines.sort((a, b) => b.averageRating - a.averageRating).slice(0, 10);
